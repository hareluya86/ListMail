...
// avoid list='all'
if($list=='all' || !$list) $list = firstlist();
...
if($efields){
 ...
 if($save){
  for($i=1;$i<=10;$i++){
   if(${"reqd".$i}){
    ${"reqd".$i} = '1';
    if(!${"error".$i}) ${"error".$i} = "Please enter your ${"field".$i}";
   }
...
list($field1,$field2,$field3,$field4,$field5,$field6,$field7,$field8,$field9,$field10)=@mysql_fetch_row($lrow);
 ...
 for($i=1;$i<=10;$i++){
  ...
  if(${"reqd".$i}=='1') ${"reqd".$i."chk"} = ' checked'; else ${"reqd".$i."chk"} = '';
 }
...
if ($doedit == 'yes') {
 if($txtcact <> '1'){ $txtcact = 0; }
 if($txtwact <> '1'){ $txtwact = 0; }
 if($txtreq1 <> '1'){ $txtreq1 = 0; }
 if($txtreq2 <> '1'){ $txtreq2 = 0; }
 if($n1chk <> '1') $n1chk = '0';
 if($n2chk <> '1') $n2chk = '0';
 if($n3chk <> '1') $n3chk = '0';
 if($n4chk <> '1') $n4chk = '0';
 if($n5chk <> '1') $n5chk = '0';
 if($closchk <> '1') $closchk = '0';
 if($cnfchk <> '1') $cnfchk = '0';
 if(!is_numeric($chtml)) $chtml = '1';
 if(!is_numeric($emsgs)) $chtml = '1';
 ...
 if(!$add_remchk) $textadd_rem = '';
 if(!$textadd_rem) $add_remchk = '0';
 $textadd_add = process_listtext($textadd_add,$textlist);  
 if(!$add_addchk) $textadd_add = '';
 if(!$textadd_add) $add_addchk = '0';
 $textrem_rem = process_listtext($textrem_rem,$textlist);  
 if(!$rem_remchk) $textrem_rem = '';
 if(!$textrem_rem) $rem_remchk = '0';
 $textrem_add = process_listtext($textrem_add,$textlist);  
 if(!$rem_addchk) $textrem_add = '';
 if(!$textrem_add) $rem_addchk = '0';
 
 if($floopchk) $floopchk = '1'; else $floopchk = '';
 if(!is_numeric($flseq) || !isset($flseq) || $flseq < 1 || !$floopchk){ list($flseq)=mysql_fetch_row(mysql_query("select seq from $ftable where list = '$list' order by seq limit 1")); if(!$flseq) $flseq = '1'; }
 if(!is_numeric($fldel) || !isset($fldel) || $fldel < 0 || !$floopchk){ list($fldel)=mysql_fetch_row(mysql_query("select del from $ftable where list = '$list' order by seq limit 1")); if(!$fldel) $fldel = '0'; }
 $floopchk .= ":$flseq:$fldel";
 
 if($floadchk && is_numeric($floadperc) && is_numeric($floadid)) $floadchk = '1'; else { $floadchk = ''; $floadperc = '100'; $floadid = ''; }
 
 if($fdelchk) $fdelchk = '1'; else $fdelchk = '';
 if($fmovechk && is_numeric($fmovelist) && is_numeric($fmoveseq) && is_numeric($fmovedel) && $fmovelist >= 1 && $fmoveseq >= 1 && $fmovedel >= 0) $fmovechk = '1'; else { $fmovechk = ''; $fmovelist = '1'; $fmoveseq = '1'; $fmovedel = '0'; }
 ...
 if($chglist){
  ...
  while(list($dupeid,$dupenum) = @mysql_fetch_row($result)){
    if(($dupenum == $textlist) && ($dupeid <> $editid)) { $duped = 1; }
  }
 }

 if($textlist=='0') $duped=1;
 if($duped) {
  $logtxt = "Duplicate or invalid list number!  Not saved!";
 } else {
  if(!$demo) {
   if($chglist) { mysql_query($cmd_fp); mysql_query($cmd_us); mysql_query($cmd_mc); }
...
if($dodel) {
 if(!$demo) {
  if(!is_numeric($delid)) exit('invalid data');
...
if(!$demo){
 ...
 $list = $listnum;
 $doadd='';
} else { $logtxt = 'Cannot add list in demo.'; }
}
...
if(!$demo){
 ...
 if($fupcopy) $logtxt .= "$fupcopy followups copied. ";
 $list = $listnum;
} else { $logtxt = 'Cannot copy list in demo.'; }
}
if ($getcode) {
...
if(!$subtype || $subtype=='menu') $rmenuchk = ' checked';
if($subtype=='radi') $rradchk = ' checked';

if($scheck == 1){
 if($subtype=='menu'){ $thecode .= "<select name=submode>\n<option value=sub>Subscribe</option>\n<option value=unsub>Remove</option>\n</select>\n"; }
 if($subtype=='radi'){ $thecode .= "<input type=radio name=submode value=sub checked>Signup\n<input type=radio name=submode value=unsub>Remove\n"; }
}

if(!$httype || $httype=='menu') $htmenuchk = ' checked';
if($httype=='radi') $htradchk = ' checked';

if($htcheck == 1){
 if($httype=='menu'){ $thecode .= "<select name=htmail>\n<option value=html>HTML</option>\n<option value=text>Text</option>\n</select>\n"; }
 if($httype=='radi'){ $thecode .= "<input type=radio name=htmail value=HTML checked>HTML\n<input type=radio name=htmail value=text>Text\n"; }
}
...
if($rqx=='1') $reqxchk = ' checked'; else $reqxchk = '';
if($cnfact==1){ $cactcheck = ' checked'; } else { $cactcheck = ''; }
if($welcact==1){ $wactcheck = ' checked'; } else { $wactcheck = ''; }
if($remact==1){ $ractcheck = ' checked'; } else { $ractcheck = ''; }
...
if(!is_numeric($flseq) || !isset($flseq) || $flseq < 1){ list($flseq)=mysql_fetch_row(mysql_query("select seq from $ftable where list = '$list' order by seq limit 1")); if(!$flseq) $flseq = '1'; }
if(!is_numeric($fldel) || !isset($fldel) || $fldel < 0){ list($fldel)=mysql_fetch_row(mysql_query("select del from $ftable where list = '$list' order by seq limit 1")); if(!$fldel) $fldel = '0'; }

list($add_rem,$add_add,$rem_rem,$rem_add,$floadperc,$floadid,$fmovelist,$fmoveseq,$fmovedel) = explode(';',$addopts);

if($xopts[0]==1) $closcheck = ' checked'; else $closcheck = '';
if($xopts[1]==1) $cnfcheck = ' checked'; else $cnfcheck = '';
if($xopts[2]==1) $add_remcheck = ' checked'; else $add_remcheck = '';
if($xopts[3]==1) $add_addcheck = ' checked'; else $add_addcheck = '';
if($xopts[4]==1) $rem_remcheck = ' checked'; else $rem_remcheck = '';
if($xopts[5]==1) $rem_addcheck = ' checked'; else $rem_addcheck = '';
if($xopts[6]==1) $floopcheck = ' checked'; else $floopcheck = '';
if($xopts[7]==1) $floadcheck = ' checked'; else $floadcheck = '';
if($xopts[8]==1) $fmovecheck = ' checked'; else $fmovecheck = '';
if($xopts[9]==1) $fdelcheck = ' checked'; else $fdelcheck = '';
...
if($nf==0) echo "<option value=x selected>$nfs";
...
if(is_numeric($floadperc)) ${"fupsel".$floadperc} = ' selected';
...
if(@mysql_num_rows($rows)>0){
 ...
 if($fsid == $floadid) $sel = ' selected'; else $sel = '';
