...
include("./config.php");
include("./admin.php");
//$csvfile = './list/RB24 list 20131221.csv';
$csvfile = './list/'.$file;
$list = 19;

if($password != 'airnav') exit;
...
$rb_db = 'rb24';
$rb_host = 'mysql.airnavsystems.com';
$rb_user = 'newsletter';
$rb_pwd = 'newsletter123*';

//to loop through
$total_count = 0;
$last_pointer = 0;
$max_count = 10000;
$rb24_cmd = 'select * from rb24.accounts';

//Get count first
try {
    $pdo_rb_db = 'mysql:dbname=' . $rb_db . ';host=' . $rb_host;
    $rb_dbh = new PDO($pdo_rb_db, $rb_user, $rb_pwd);
    $rb_pdo_query = $rb_dbh->query($rb24_cmd);
    $total_count = $rb_pdo_query->rowCount();
    ...
    while (list($email) = fgetcsv($csv)) {
        ...
        //echo 'confirm:'.$confirmed.'<br>';//debug
            ...
            if ($remote) { //open connection to remote host
                //if (!$dbh) {
                echo 'Connecting to remote list <br>'; //debug
                    try {
                        $pdo_db = 'mysql:dbname=' . $remotedb . ';host=' . $remotehost;
                        $dbh = new PDO($pdo_db, $remoteuser, $remotepwd);
                    } catch (PDOException $e) {
            ...
            if ($cnfact == '1' || ($cnfact <> '1' && $welcact == '1')) {
                sendwelcome($userid, $list); echo "oh no we're sending welcome!<br>";//debug
            }
        ...
        if (!$cgi) {echo "Display custom success page.. <br>";
            ...
            echo 'chtml params: '.$t.' '.$i.'<br>';//debug
            $data = chtml($confirmed, $chid); echo 'calling chtml result: '.$data.'<br>';
            ...
            if ($key == 0)
                if (strpos(' ' . $data, 'URL:') == 1)
                    ...
                    echo 'end of the file'.$data.' for email '.$email.' <br>';
            //echo 'key ' . $key;
        }
    }
